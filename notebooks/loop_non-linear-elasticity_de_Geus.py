...
# ----------------------------------- GRID ------------------------------------
total_number_CG_per_grid_size=[]
for Nx in [9,15,25,35,55]: #,55,65,75
    ...
    while True:
        ...
        def callback(x):
            ...
            #print(f'rr norm = { np.linalg.norm( G_K_deps(x)-b)}')
    ...
    total_number_CG_per_grid_size.append(np.sum(nb_iter_CG_per_Newthon))
    ...
    ax[0].semilogy(norm_fluctuation/ En)
    ax[0].set_xlabel(r'Newton iteration - $i$')
    ax[0].set_ylabel(r'$\|\mathsf{B} \delta\tilde{\mathsf{u}}^{(i+1)}\| / \|\mathsf{E}\|$')

    ax[1].plot(nb_iter_CG_per_Newthon)
    ax[1].set_xlabel(r'Newton iteration -  $i$')
    ax[1].set_ylabel(r'Number of PCG iterations')
    plt.show()

fig, ax = plt.subplots(1, 2)

ax[0].plot(total_number_CG_per_grid_size)
ax[0].set_ylabel('Total number of PCG iterations')
ax[0].set_xlabel(r'grid size')
